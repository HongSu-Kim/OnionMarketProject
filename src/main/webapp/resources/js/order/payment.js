...
$('#delivery-true').click(function () {
	...
	$('#orderPaymentSpan').html(parseInt($('#orderPaymentSpan').html()) + 3000)
})
...
let blurUsePoint = () => {
	...
	let pointRegex = /^[0-9]*$/
	if (!pointRegex.test($('#usePoint').val())) {
		$('#point').before('<li class="err-msg point-err">&nbsp;<span class="err-msg">숫자만 입력해주세요.</span></li>')
		return false;
	} else if (parseInt($('#usePoint').val()) > parseInt($('#point').val())) {
		$('#point').before('<li class="err-msg point-err">&nbsp;<span class="err-msg">보유 포인트만 사용가능합니다.</span></li>')
		$('#usePoint').val($('#point').val())
		$('#orderPaymentSpan').html(parseInt($('#totalPrice').html()) - $('#usePoint').val())
		return false;
	} else if ($('#usePoint').val() == "") {
		$('#usePoint').val(0)
	}

	$('#orderPaymentSpan').html(parseInt($('#totalPrice').html()) - $('#usePoint').val())
	$('#orderPayment').val(parseInt($('#totalPrice').html()) - $('#usePoint').val())
	return true;
}
...
$('#payment').click(function () {
	...
	// $('#orderPayment').val($('#totalPrice').html() - $('#usePoint').val())
	$('#form').submit()
})
...
let validate = () => {
	...
	if ($('#delivery').val() == "true") {
		...
		if (!$('#deliveryTel').val()) {
			...
			result = true;
		} else if(!telRegex.test($('#deliveryTel').val())) {
			$('#postcode').before('<span class="err-msg">전화번호 형식이 맞지않습니다.</span>')
			result = true;
		}
