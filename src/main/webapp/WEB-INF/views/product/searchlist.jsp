			...
			<c:if test="${!empty page.content && page.totalPages != 1}">
				...
				<div class="product__pagination text-center">
					...
					<c:if test="${page.totalPages > block && page.number + 1 > block}">
						<a href="?searchName=${searchName}&page=0"><<</a>
						<a href="?searchName=${searchName}&page=${startNumber - 1}"><</a>
					</c:if>
					<c:forEach var="currentNumber" begin="${startNumber}" end="${endNumber}">
						<a href="?searchName=${searchName}&page=${currentNumber}">${currentNumber + 1}</a>
					</c:forEach>
					<c:if test="${page.totalPages - 1 > endNumber}">
						<a href="?searchName=${searchName}&page=${endNumber + 1}">></a>
						<a href="?searchName=${searchName}&page=${page.totalPages - 1}">>></a>
					</c:if>
				</div>
			</c:if>
